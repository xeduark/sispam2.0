...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Procesar y clasificar archivos subidos
    $archivos_adjuntos = [];
    $tipos_adjuntados = [];
    $tipos_subidos = $_POST['doc_tipo_categoria'] ?? [];

    if (isset($_FILES['doc_archivos'])) {
        foreach ($_FILES['doc_archivos']['name'] as $idx => $original_name) {
            if (!empty($original_name) && $_FILES['doc_archivos']['error'][$idx] === UPLOAD_ERR_OK) {
                $cat_tag = $tipos_subidos[$idx] ?? 'OTRO';
                $tipos_adjuntados[] = $cat_tag;
                $archivos_adjuntos[$cat_tag] = [
                    'name'     => $original_name,
                    'tmp_name' => $_FILES['doc_archivos']['tmp_name'][$idx],
                    'type'     => $_FILES['doc_archivos']['type'][$idx]
                ];
            }
        }
    }
...
<script src="assets/js/scanner_doc.js"></script>
