...
@section('titulo', 'Entrega & Factura - '.config('app.name'))
...
<div class="card card-glass border-0 shadow-sm">
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        ...
        <span class="badge bg-info text-dark fs-6">{{ $listaEntrega->count() }} Esperando Entrega</span>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    @foreach ($listaEntrega as $row)
                    <tr>
                        <td class="ps-3 fw-bold text-primary fs-5">{{ $row->ticket_numero }}</td>
                        <td><span class="badge bg-success fs-6"><i class="fa-solid fa-door-open me-1"></i> {{ $row->modulo_entrega_asignado }}</span></td>
                        <td>
                            <div class="fw-bold">
                        ...
                        <td>
                            @if (!empty($row->faltantes_alistamiento))
                        ...
                        <td>
                            <div class="d-flex gap-1 flex-wrap">
                                @if (!empty($row->pdf_transcripcion_url))
                                    <a href="{{ asset($row->pdf_transcripcion_url) }}" target="_blank" class="btn btn-sm btn-outline-danger" title="Ver PDF Transcripción">
                                        <i class="fa-solid fa-file-pdf me-1"></i> Transcripción
                                    </a>
                                @endif
                                @if (!empty($row->pdf_alistamiento))
                                    <a href="{{ asset($row->pdf_alistamiento) }}" target="_blank" class="btn btn-sm btn-outline-success" title="Ver PDF Alistamiento">
                        ...
                        <td class="text-end pe-3">
                            <button type="button" 
                                    ...
                                    data-paciente="{{ $row->paciente->nombres . ' ' . $row->paciente->apellidos }}"
                                    data-doc="{{ $row->paciente->tipo_documento . ' ' . $row->paciente->numero_documento }}"
                                    data-faltantes="{{ $row->faltantes_alistamiento ?? '' }}">
...
<div class="modal fade" id="modalFirmaDigital" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content card-glass">
            ...
            <form method="POST" action="{{ route('entrega.finalizar') }}" enctype="multipart/form-data" id="formEntregaFirma">
                @csrf
